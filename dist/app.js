...
router.get('/parse', async (ctx) => {
    ...
    // 请求源数据页面
    const rawData = await fetchSource(url);
    ...
    // 解析各特别行政区源数据
    const sars = ['macau', 'hongkong', 'taiwan'];
    ...
    await Promise.all(sars.map(async (file) => {
        await readJSON(file).then((data) => {
            console.log(`${file} contains: `, `${data.province?.length ?? 0} provinces`, `${data.city?.length ?? 0} cities`, `${data.county?.length ?? 0} counties`);
            // 合并数据集入 dataset
